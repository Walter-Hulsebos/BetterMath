...
namespace CGTK.Utilities.BetterMath
{
    public static partial class noise
    {
        ...
        public static F32x2 cellular2x2x2(F32x3 P)
        {
            ...
            F32x3 Pf = frac(P);
            F32x4 Pfx = Pf.x + new F32x4(0.0f, -1.0f, 0.0f, -1.0f);
            F32x4 Pfy = Pf.y + new F32x4(0.0f, 0.0f, -1.0f, -1.0f);
            F32x4 p = permute(Pi.x + new F32x4(0.0f, 1.0f, 0.0f, 1.0f));
            p = permute(p + Pi.y + new F32x4(0.0f, 0.0f, 1.0f, 1.0f));
            F32x4 p1 = permute(p + Pi.z); // z+0
            F32x4 p2 = permute(p + Pi.z + new F32x4(1.0f,1.0f,1.0f,1.0f)); // z+1
            F32x4 ox1 = frac(p1 * K) - Ko;
            F32x4 oy1 = mod7(floor(p1 * K)) * K - Ko;
            F32x4 oz1 = floor(p1 * K2) * Kz - Kzo; // p1 < 289 guaranteed
            F32x4 ox2 = frac(p2 * K) - Ko;
            F32x4 oy2 = mod7(floor(p2 * K)) * K - Ko;
            F32x4 oz2 = floor(p2 * K2) * Kz - Kzo;
            F32x4 dx1 = Pfx + jitter * ox1;
            F32x4 dy1 = Pfy + jitter * oy1;
            F32x4 dz1 = Pf.z + jitter * oz1;
            F32x4 dx2 = Pfx + jitter * ox2;
            F32x4 dy2 = Pfy + jitter * oy2;
            F32x4 dz2 = Pf.z - 1.0f + jitter * oz2;
            F32x4 d1 = dx1 * dx1 + dy1 * dy1 + dz1 * dz1; // z+0
            F32x4 d2 = dx2 * dx2 + dy2 * dy2 + dz2 * dz2; // z+1

            // Sort out the two smallest distances (F1, F2)

            // Do it right and sort out both F1 and F2
            F32x4 d = min(d1,d2); // F1 is now in d
