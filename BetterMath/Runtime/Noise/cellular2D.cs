...
namespace CGTK.Utilities.BetterMath
{
    public static partial class noise
    {
        ...
        public static F32x2 cellular(F32x2 P)
        {
            ...
            F32x2 Pf = frac(P);
            F32x3 oi = new F32x3(-1.0f, 0.0f, 1.0f);
            F32x3 of = new F32x3(-0.5f, 0.5f, 1.5f);
            F32x3 px = permute(Pi.x + oi);
            F32x3 p = permute(px.x + Pi.y + oi); // p11, p12, p13
            F32x3 ox = frac(p * K) - Ko;
            F32x3 oy = mod7(floor(p * K)) * K - Ko;
            F32x3 dx = Pf.x + 0.5f + jitter * ox;
            F32x3 dy = Pf.y - of + jitter * oy;
            F32x3 d1 = dx * dx + dy * dy; // d11, d12 and d13, squared
            p = permute(px.y + Pi.y + oi); // p21, p22, p23
            ox = frac(p * K) - Ko;
            oy = mod7(floor(p * K)) * K - Ko;
            dx = Pf.x - 0.5f + jitter * ox;
            dy = Pf.y - of + jitter * oy;
            F32x3 d2 = dx * dx + dy * dy; // d21, d22 and d23, squared
            p = permute(px.z + Pi.y + oi); // p31, p32, p33
            ox = frac(p * K) - Ko;
            oy = mod7(floor(p * K)) * K - Ko;
            dx = Pf.x - 1.5f + jitter * ox;
            dy = Pf.y - of + jitter * oy;
            F32x3 d3 = dx * dx + dy * dy; // d31, d32 and d33, squared
