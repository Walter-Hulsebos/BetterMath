//
// F32x3  psrdnoise(F32x2 pos, F32x2 per, float rot)
// F32x3  psrdnoise(F32x2 pos, F32x2 per)
// float psrnoise(F32x2 pos, F32x2 per, float rot)
// float psrnoise(F32x2 pos, F32x2 per)
// F32x3  srdnoise(F32x2 pos, float rot)
// F32x3  srdnoise(F32x2 pos)
// float srnoise(F32x2 pos, float rot)
// float srnoise(F32x2 pos)
//
...
//
// F32x3 {p}s{r}dnoise(F32x2 pos {, F32x2 per} {, float rot})
...
//
// float {p}s{r}noise(F32x2 pos {, F32x2 per} {, float rot})
// "pos" is the input (x,y) coordinate
// "per" is the x and y period, where per.x is a positive integer
//    and per.y is a positive even integer
// "rot" is the angle to rotate the gradients (any float value,
...
namespace CGTK.Utilities.BetterMath
{
    public static partial class noise
    {
        ...
        public static F32x3 psrdnoise(F32x2 pos, F32x2 per, float rot)
        {
            // Hack: offset y slightly to hide some rare artifacts
            pos.y += 0.01f;
            // Skew to hexagonal grid
            F32x2 uv = new F32x2(pos.x + pos.y * 0.5f, pos.y);

            F32x2 i0 = floor(uv);
            F32x2 f0 = frac(uv);
            // Traversal order
            F32x2 i1 = (f0.x > f0.y) ? new F32x2(1.0f, 0.0f) : new F32x2(0.0f, 1.0f);

            // Unskewed grid points in (x,y) space
            F32x2 p0 = new F32x2(i0.x - i0.y * 0.5f, i0.y);
            F32x2 p1 = new F32x2(p0.x + i1.x - i1.y * 0.5f, p0.y + i1.y);
            F32x2 p2 = new F32x2(p0.x + 0.5f, p0.y + 1.0f);
            ...
            F32x3 xw = fmod(new F32x3(p0.x, p1.x, p2.x), per.x);
            F32x3 yw = fmod(new F32x3(p0.y, p1.y, p2.y), per.y);
            F32x3 iuw = xw + 0.5f * yw;
            F32x3 ivw = yw;

            // Create gradients from indices
            F32x2 g0 = rgrad2(new F32x2(iuw.x, ivw.x), rot);
            F32x2 g1 = rgrad2(new F32x2(iuw.y, ivw.y), rot);
            F32x2 g2 = rgrad2(new F32x2(iuw.z, ivw.z), rot);

            // Gradients math.dot vectors to corresponding corners
            // (The derivatives of this are simply the gradients)
            F32x3 w = new F32x3(dot(g0, d0), dot(g1, d1), dot(g2, d2));

            // Radial weights from corners
            // 0.8 is the square of 2/math.sqrt(5), the distance from
            // a grid point to the nearest simplex boundary
            F32x3 t = 0.8f - new F32x3(dot(d0, d0), dot(d1, d1), dot(d2, d2));

            // Partial derivatives for analytical gradient computation
            F32x3 dtdx = -2.0f * new F32x3(d0.x, d1.x, d2.x);
            F32x3 dtdy = -2.0f * new F32x3(d0.y, d1.y, d2.y);
            ...
            F32x2 dt0 = new F32x2(dtdx.x, dtdy.x) * 4.0f * t3.x;
            F32x2 dn0 = t4.x * g0 + dt0 * w.x;
            F32x2 dt1 = new F32x2(dtdx.y, dtdy.y) * 4.0f * t3.y;
            F32x2 dn1 = t4.y * g1 + dt1 * w.y;
            F32x2 dt2 = new F32x2(dtdx.z, dtdy.z) * 4.0f * t3.z;
            F32x2 dn2 = t4.z * g2 + dt2 * w.z;

            return 11.0f * new F32x3(n, dn0 + dn1 + dn2);
        }
        ...
        /// <param name="per">The x and y period, where per.x is a positive integer and per.y is a positive even integer.</param>
        /// <returns>The first component of the 3-element return vector is the noise value, and the second and third components are the x and y partial derivatives.</returns>
        public static F32x3 psrdnoise(F32x2 pos, F32x2 per)
        {
        ...
        public static float psrnoise(F32x2 pos, F32x2 per, float rot)
        {
        ...
        /// <param name="per">The x and y period, where per.x is a positive integer and per.y is a positive even integer.</param>
        /// <returns>Noise value.</returns>
        public static float psrnoise(F32x2 pos, F32x2 per)
        {
        ...
        /// <param name="rot">Angle to rotate the gradients.</param>
        /// <returns>The first component of the 3-element return vector is the noise value, and the second and third components are the x and y partial derivatives.</returns>
        public static F32x3 srdnoise(F32x2 pos, float rot)
        {
            // Offset y slightly to hide some rare artifacts
            pos.y += 0.001f;
            // Skew to hexagonal grid
            ...
            F32x3 x = new F32x3(p0.x, p1.x, p2.x);
            F32x3 y = new F32x3(p0.y, p1.y, p2.y);
            F32x3 iuw = x + 0.5f * y;
            F32x3 ivw = y;
        ...
        /// <param name="pos">Input (x,y) coordinate.</param>
        /// <returns>The first component of the 3-element return vector is the noise value, and the second and third components are the x and y partial derivatives.</returns>
        public static F32x3 srdnoise(F32x2 pos)
        {
        ...
        /// <param name="rot">Angle to rotate the gradients.</param>
        /// <returns>Noise value.</returns>
        public static float srnoise(F32x2 pos, float rot)
        {
            // Offset y slightly to hide some rare artifacts
            pos.y += 0.001f;
            // Skew to hexagonal grid
        ...
        /// <param name="pos">Input (x,y) coordinate.</param>
        /// <returns>Noise value.</returns>
        public static float srnoise(F32x2 pos)
        {
