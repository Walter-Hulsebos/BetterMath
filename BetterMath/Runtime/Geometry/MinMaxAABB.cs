...
namespace CGTK.Utilities.BetterMath.Geometry
{
    ...
    internal struct MinMaxAABB : IEquatable<MinMaxAABB>
    {
        ...
        public MinMaxAABB(F32x3 min, F32x3 max)
        {
            Min = min;
            Max = max;
        }
        ...
        /// If you have F16 extents, you can call <see cref="CreateFromCenterAndHalfExtents"/>.
        /// </remarks>
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static MinMaxAABB CreateFromCenterAndExtents(F32x3 center, F32x3 extents)
        {
            return CreateFromCenterAndHalfExtents(center, extents * 0.5f);
        }

        /// <summary>
        /// Creates the AABB from a center and F16 extents.
        /// </summary>
        /// <remarks>
        /// This function takes F16 extents. It is F16 the distance between <see cref="Min"/> and <see cref="Max"/>.
        ...
        /// <returns>AABB created from inputs.</returns>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static MinMaxAABB CreateFromCenterAndHalfExtents(F32x3 center, F32x3 halfExtents)
        {
        ...
        /// </remarks>
        public F32x3 Extents => Max - Min;

        /// <summary>
        /// Computes the F16 extents of the AABB.
        /// </summary>
        /// <remarks>
        /// HalfExtents is F16 of the componentwise distance between min and max. Subtracting HalfExtents from Center
        /// gives Min and adding HalfExtents to Center gives Max.
        /// </remarks>
        public F32x3 HalfExtents => (Max - Min) * 0.5f;

        /// <summary>
        /// Computes the center of the AABB.
        /// </summary>
        public F32x3 Center => (Max + Min) * 0.5f;
        ...
        public bool IsValid => BetterMath.Maths.all(Min <= Max);
        ...
        public float SurfaceArea
        {
            ...
            {
                F32x3 diff = Max - Min;
                return 2 * BetterMath.Maths.dot(diff, diff.yzx);
            }
        }
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public bool Contains(F32x3 point) => BetterMath.Maths.all(point >= Min & point <= Max);
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public bool Contains(MinMaxAABB aabb) => BetterMath.Maths.all((Min <= aabb.Min) & (Max >= aabb.Max));
        ...
        {
            return BetterMath.Maths.all(Max >= aabb.Min & Min <= aabb.Max);
        }
        ...
        /// </remarks>
        /// <seealso cref="Contains(CGTK.Utilities.BetterMath.Geometry.MinMaxAABB)"/>
        /// <param name="aabb">AABB to encapsulate.</param>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public void Encapsulate(MinMaxAABB aabb)
        {
            Min = BetterMath.Maths.min(Min, aabb.Min);
            Max = BetterMath.Maths.max(Max, aabb.Max);
        }
        ...
        /// then this AABB doesn't change.
        /// </remarks>
        /// <seealso cref="Contains(F32x3)"/>
        /// <param name="point">Point to encapsulate.</param>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public void Encapsulate(F32x3 point)
        {
            Min = BetterMath.Maths.min(Min, point);
            Max = BetterMath.Maths.max(Max, point);
        }
    ...
    internal static partial class Math
    {
        ...
        {
            F32x3 halfExtentsInA = aabb.HalfExtents;

            // Rotate each axis individually and find their new positions in the rotated space.
            F32x3 x = BetterMath.Maths.rotate(transform.rot, new F32x3(halfExtentsInA.x, 0, 0));
            F32x3 y = BetterMath.Maths.rotate(transform.rot, new F32x3(0, halfExtentsInA.y, 0));
            F32x3 z = BetterMath.Maths.rotate(transform.rot, new F32x3(0, 0, halfExtentsInA.z));
            ...
            F32x3 halfExtentsInB = BetterMath.Maths.abs(x) + BetterMath.Maths.abs(y) + BetterMath.Maths.abs(z);
            F32x3 centerInB = BetterMath.Maths.transform(transform, aabb.Center);
        ...
        /// <returns>Transformed AABB.</returns>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static MinMaxAABB Transform(F32x4x4 transform, MinMaxAABB aabb)
        {
            var transformed = Transform(new F32x3x3(transform), aabb);
        ...
        /// <returns>Transformed AABB.</returns>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static MinMaxAABB Transform(F32x3x3 transform, MinMaxAABB aabb)
        {
